                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <div>
                            <select name="status" class="px-3 py-2 border rounded">
                                ...
                                <option value="0" {{ $search_params['status'] === '0' ? 'selected' : '' }}>承認済み</option>
                                <option value="1" {{ $search_params['status'] === '1' ? 'selected' : '' }}>未承認</option>
                            </select>
                        </div>
                            ...
                            <tbody class="bg-white">
                                @foreach($expenses as $expense)
                                    <tr class="border-b border-gray-200 hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right border-r">¥{{ number_format($expense->total_amount) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{-- ステータスバッチ 条件分岐 --}}
                                            @if($expense->freee_sync_status === 0)
